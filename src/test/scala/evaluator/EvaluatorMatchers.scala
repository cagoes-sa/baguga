...
trait EvaluatorMatchers {
  ...
  def beEvaluatedWithType(objectType: ObjectType) =
    new TypeEvaluatorMatcher(objectType)

  def beEqualTo[T](value: T) = new ValueEvaluatorMatcher(value: T)
  def beInspectedInto(value: String) =
    new InspectionMatcher(expectedString = value)
  ...
  class InspectionMatcher(expectedString: String) extends Matcher[String] {
    def apply(input: String): MatchResult = {
      ...
      MatchResult(
        ...
        s"""'$input' should be inspected into '$expectedString' but it's actually ${value
             .getOrElse(NullObject)
             .inspect}
           | also, the following errors were found when parsing the code: ${parser.errors
             .map(_.message)
             .mkString("\n\t")}
           |""".stripMargin,
        s"The program $input returned the right type!"
  ...
  class TypeEvaluatorMatcher(objectType: ObjectType) extends Matcher[String] {
    def apply(input: String): MatchResult = {
      ...
      MatchResult(
        ...
        s"""'$input' be evaluated for type ${objectType.toString} but instead became ${value
             .getOrElse(NullObject)
             .objectType}
           | also, the following errors were found when parsing the code: ${parser.errors
             .map(_.message)
